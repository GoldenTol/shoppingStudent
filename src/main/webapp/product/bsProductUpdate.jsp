...
<%
	...
	// request라는 scope에 su와 xxx라는 attribute를 바인딩 */
	/* 내장 객체 중 일부(특히 request)는 자체 저장소가 있다. 저장소라고 하자 
	왼쪽은 식별자(여기서는 su, xxx) 오른쪽은 속성(여기서는 12345, bean) */
	/* 12345를 저장하는데 su라는 이름을 붙여서 넣었다. 다시말해
	xxx라는 이름으로 bean을 집어넣었다고 이해하면 쉽다. (고기칸(request) 열어서 삼겹살(su)중 230215일자 삼겹살(12345)꺼내듯이) */
	
	// getAttribute의 반환타입은 object지만 실제로는 정수타입(다시말해 강등)
	int su = (Integer)request.getAttribute("su"); // auto unboxing
	Product yyy = (Product)request.getAttribute("xxx"); // auto unboxing
	// Product bean = dao.getDataByPk(10); ← 여기서 product였기 때문에 넣을 때 object로 바뀐것을 꺼낼 때 다시 product로 강등
		...
		<link rel="stylesheet" href="//code.jquery.com/ui/1.13.2/themes/base/jquery-ui.css">
 		...
 		<script type="text/javascript">
 			$(document).ready(function(){
 				...
 				/* 카테고리 확인 */
 				var category = '${requestScope.xxx.category}';
 				/* 
 				...
 				*/ 
 				...
 				/* ↑ 랑 같은 값
 				$("input[value='<%=bean.getCategory()%>']").attr('selected', true); 
 				*/
 				/* child를 체크한다. */
